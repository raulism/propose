...
function animate() {
	...
	if (newTime - lastTime > 500 + (window.innerHeight - 767) / 2) {
		...
		if (random) {
			var bigboom = new Boom(getRandom(canvas.width / 3,
					canvas.width * 2 / 3), 2, "#FFF", {
				x : x,
				y : y
			});
			...
			var bigboom = new Boom(getRandom(canvas.width / 3,
				...
				x : canvas.width / 2,
				y : 200
			}, document.querySelectorAll(".shape")[parseInt(getRandom(0,
...
function drawMoon() {
	...
	for ( var i = 0; i < 10; i++) {
		ctx.save();
		ctx.beginPath();
		ctx.arc(centerX + width / 2, centerY + width / 2, width / 2 + index, 0,
				2 * Math.PI);
...
canvas.onclick = function() {
	var x = event.clientX;
	var y = event.clientY;
	var bigboom = new Boom(getRandom(canvas.width / 3, canvas.width * 2 / 3),
			2, "#FFF", {
				x : x,
				y : y
			});
	...
	_shapBoom : function() {
		...
		putValue(ocas, octx, this.shape, 5, function(dots) {
			...
			for ( var i = 0; i < dots.length; i++) {
				...
				var y = dots[i].y;
				var radius = 1;
				var frag = new Frag(that.x, that.y, radius, color, x - dx, y
						- dy);
...
function putValue(canvas, context, ele, dr, callback) {
	...
	if (ele.innerHTML.indexOf("img") >= 0) {
		...
		context.save();
		var fontSize = 150;
		context.font = fontSize + "px ËÎÌו";
		context.textAlign = "center";
		context.textBaseline = "middle";
		context.fillStyle = "rgba(" + parseInt(getRandom(128, 255)) + ","
...
Frag.prototype = {
	paint : function() {
		...
		ctx.fillStyle = "rgba(" + this.color.a + "," + this.color.b + ","
				+ this.color.c + ",1)";
		ctx.fill();
		ctx.restore();
	},
